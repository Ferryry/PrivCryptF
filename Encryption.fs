...
type EncryptionBaseClass =

    static member HashKey(input : string) : byte[] =
        use sha256 = SHA256.Create()
        sha256.ComputeHash(Encoding.UTF8.GetBytes(input))

    static member GenerateSalt() : byte[] =
    ...
    static member Encrypt(file : string, key : string) : unit = 
        let fs              = new FileStream(String.Format("{0}.aesfs", file), FileMode.Create)
        let password        = EncryptionBaseClass.HashKey(key)
        let salt            = EncryptionBaseClass.GenerateSalt()
    ...
    static member Decrypt(file : string, key : string) : unit =
